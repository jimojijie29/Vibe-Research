...
export async function downloadReport(id: string, name: string): Promise<void> {
  const resp = await fetch(`/api/myreports/file/${id}`, {
    headers: authHeaders(),
  });
...
export interface ValPercentile {
  period: string;
  metrics: { pe_ttm?: ValMetric; pb?: ValMetric };
}
...
export interface MarketSnapshot {
  ...
  margin_balance: {
    sh_rzye: number | null;
    sh_rqye: number | null;
    sh_rzrqye: number | null;
    sz_rzye: number | null;
    sz_rqye: number | null;
    sz_rzrqye: number | null;
    sh_rzye_change: number | null;
    sz_rzye_change: number | null;
    total_rzye_change: number | null;
...
export const api = {
  ...
  globalStock: (symbol: string) =>
    get<GlobalStock>(`/global/stock?symbol=${encodeURIComponent(symbol)}`),
  globalQuotes: (symbols: string) =>
    get<GlobalBatchQuote[]>(
      `/global/quotes?symbols=${encodeURIComponent(symbols)}`,
    ),
  ...
  marginStockRank: (top = 10, date?: string) =>
    get<MarginRank>(
      `/margin/stock-rank?top=${top}${date ? `&date=${encodeURIComponent(date)}` : ""}`,
    ),
  marginSectorRank: (top = 10, date?: string) =>
    get<MarginRank>(
      `/margin/sector-rank?top=${top}${date ? `&date=${encodeURIComponent(date)}` : ""}`,
    ),
  radar: () => get<RadarData>("/radar"),
  radarRefresh: () => request<RadarData>("/radar/refresh", "POST"),
  portfolio: () => get<PortfolioData>("/portfolio"),
  addHolding: (code: string, shares: number, cost: number) =>
    request<PortfolioData>("/portfolio/holding", "POST", {
      code,
      shares,
      cost,
    }),
  removeHolding: (code: string) =>
    request<PortfolioData>(`/portfolio/holding?code=${code}`, "DELETE"),
  ...
  closePosition: (
    ...
    request<PortfolioData>("/portfolio/close", "POST", {
      code,
      date,
      price,
      shares,
  ...
  removeClosed: (index: number) =>
    request<PortfolioData>(`/portfolio/close?index=${index}`, "DELETE"),
  valuation: (code: string) => get<Valuation>(`/valuation?code=${code}`),
  percentile: (code: string) =>
    get<ValPercentile>(`/valuation/percentile?code=${code}`),
  financials: (code: string) => get<Financials>(`/financials?code=${code}`),
  announcements: (code: string) =>
    get<Announcement[]>(`/announcements?code=${code}`),
  ...
  blockTrade: (code: string) =>
    get<BlockTradeRow[]>(`/block-trade?code=${code}`),
  ...
  hotConcepts: (code: string) =>
    get<HotConcept[]>(`/hot-concepts?code=${code}`),
  ...
  deleteReport: (id: string) =>
    request<{ ok: boolean }>(`/myreports/${id}`, "DELETE"),
};
