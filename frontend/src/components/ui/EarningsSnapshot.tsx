...
export function EarningsSnapshot({ val, fin, pctl }: Props) {
  ...
  if (revYoy != null)
    tags.push(
      `营收${revYoy >= 30 ? "高增长" : revYoy >= 0 ? "正增长" : "下滑"}`,
    );
  if (revYoy != null && npYoy != null)
    tags.push(npYoy >= revYoy ? "利润增速快于营收" : "利润增速慢于营收");
  if (roe != null)
    tags.push(`${roe >= 15 ? "高" : roe >= 8 ? "中" : "偏低"} ROE ${roe}%`);
  if (pePctile != null)
    tags.push(
      `PE ${pePctile < 30 ? "低" : pePctile <= 70 ? "中" : "高"}分位 ${Math.round(pePctile)}%`,
    );
  ...
  if (val.digest_years != null && val.digest_years > 0)
    fwd.push(`估值消化 ${val.digest_years} 年`);
    ...
    <GlassCard glow className="mb-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-lg bg-muted/30 p-3">
          ...
          {fin.revenue_yoy && (
            <p className={cn("text-xs", yoyColor(fin.revenue_yoy))}>
              同比 {fin.revenue_yoy}
            </p>
          )}
        </div>
        <div className="rounded-lg bg-muted/30 p-3">
          ...
          {fin.net_profit_yoy && (
            <p className={cn("text-xs", yoyColor(fin.net_profit_yoy))}>
              同比 {fin.net_profit_yoy}
            </p>
          )}
        </div>
      </div>
      ...
      {fwd.length > 0 && (
        <p className="mt-3 text-xs text-muted-foreground">
          <span className="text-muted-foreground/60">前向预期：</span>
          {fwd.join(" · ")}
        </p>
      )}
    </GlassCard>
