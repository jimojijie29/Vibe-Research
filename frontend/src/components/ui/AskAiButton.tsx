...
export function AskAiButton({
  ...
  const send = async (text: string) => {
    ...
    const patchLast = (
      fn: (
        msg: ChatMsg & { tools?: ToolUse[] },
      ) => ChatMsg & { tools?: ToolUse[] },
    ) =>
      setMsgs((m) =>
        m.map((msg, i) =>
          i === m.length - 1 && msg.role === "assistant" ? fn(msg) : msg,
        ),
      );
    abortRef.current?.abort();
    ...
    try {
          ...
          onTool: (tool, args) => {
            if (alive())
              patchLast((msg) => ({
                ...
                tools: [
                  ...(msg.tools || []),
                  { name: tool, arg: argStr(args) },
                ],
              }));
          },
          onDelta: (t) => {
            if (alive())
              patchLast((msg) => ({ ...msg, content: msg.content + t }));
          },
        },
        ac.signal,
      );
      ...
      if (!ac.signal.aborted)
        setErr(e instanceof ApiError ? e.message : "对话失败");
    } finally {
                        ...
                        {m.role === "assistant" &&
                          ...
                          !(loading && i === msgs.length - 1) && (
                            <div className="mt-1.5">
                              <SaveNoteButton
                                kind="问AI"
                                title={`问 AI · ${msgs[i - 1]?.content?.slice(0, 24) || "对话"}`}
                                content={m.content}
                              />
                            </div>
                          )}
