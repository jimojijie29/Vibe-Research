...
export function DailyReview() {
  ...
  const loadReviewData = () => {
    ...
    api
      ...
      .then(setEmotion)
      .catch(() => {})
      .finally(() => setEmoDone(true));
    api
      .turnoverTop()
      .then(setTurnover)
      .catch(() => {})
      .finally(() => setToDone(true));
  };
  ...
  const pending = (done: boolean) => (
    <p className="py-4 text-center text-sm text-muted-foreground/60">
      {done
        ? "暂无数据：可能是非交易时段或数据源暂时不可用，可点「刷新」重试"
        : "加载中…"}
    </p>
  );
                ...
                <>
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                    {[
                      ...
                      {
                        k: "最高连板",
                        v: `${emotion.max_boards} 板`,
                        cls: "text-primary",
                      },
                      {
                        k: "连板（2板+）",
                        v: `${emotion.lianban_count} 家`,
                        cls: "text-primary",
                      },
                    ].map((c) => (
                      <div
                        key={c.k}
                        className="rounded-lg bg-muted/25 p-3 text-center"
                      >
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-2">
                    {[
                      ...
                      {
                        k: "炸板率",
                        v: emotion.break_rate,
                        hint: "炸板 / 尝试涨停",
                        strong: false,
                      },
                      {
                        k: "晋级率",
                        v: emotion.promotion_rate,
                        hint: "昨涨停今又停",
                        strong: true,
                      },
                    ].map((c) => (
                      ...
                      >
                        ...
                        <p
                          className={cn(
                            "mt-0.5 font-mono text-sm font-bold",
                            c.strong ? "text-danger" : "text-success",
                          )}
                        >
                          ...
                          <tbody>
                            {emotion.lianban_stocks.map((s) => (
                              ...
                              >
                                ...
                                <td className="px-2 py-2 font-mono text-danger">
                                  +{s.pct}%
                                </td>
                    ...
                    <tbody>
                      {turnover.stocks.map((s, i) => (
                        <tr key={s.code} className="border-b border-border/30">
                          <td className="px-2 py-2 font-mono text-xs text-muted-foreground/50">
                            {i + 1}
                          </td>
                          <td className="px-2 py-2">
                            <span className="font-medium">{s.name}</span>{" "}
              ...
              {[
                {
                  ...
                  icon: TrendingUp,
                  color: "text-danger",
                  rows: sectors.slice(0, 6),
                },
                {
                  title: "流出 Top",
                  icon: TrendingDown,
                  color: "text-success",
                  rows: [...sectors].slice(-6).reverse(),
                },
              ].map((col) => (
                  ...
                  {col.rows.length === 0 ? (
                    ...
                    <div className="space-y-1.5">
                      {col.rows.map((s, i) => (
                        ...
                        >
                          <span className="w-5 text-xs text-muted-foreground/50">
                            {i + 1}
                          </span>
                          <span className="flex-1 truncate">{s.name}</span>
                          <span
                            className={cn("font-mono text-xs", pctColor(s.pct))}
                          >
                            {s.pct > 0 ? "+" : ""}
                            {s.pct}%
                          </span>
