  ...
  const aiContext = totals
    ? `我的持仓（本地数据）：\n` +
      holdings
        .map(
          (h) =>
            `${h.name}(${h.code}) ${h.shares}股 成本${h.cost} 现价${h.price} 浮盈${h.pnl}(${h.pnl_pct}%)`,
        )
        .join("\n") +
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-start gap-2 rounded-lg border border-success/25 bg-success/5 p-3 text-xs text-muted-foreground">
        ...
        <span>
          持仓<b className="text-foreground">只存在你本地</b>
          ，不上传、不进仓库。行情每半小时自动刷新，也可手动刷新。本产品不提供标的、不给建议，只帮你把自己的账理清楚。
        </span>
      </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <GlassCard key={m.k} className="p-3">
              ...
              <p className={cn("mt-1 font-mono text-lg font-bold", m.c)}>
                {m.v}
              </p>
            </GlassCard>
          ))}
              ...
              <tbody>
                {holdings.map((h) => (
                  <tr key={h.code} className="border-b border-border/30">
                    ...
                    <td
                      className={cn("px-2 py-2.5 font-mono", pnlColor(h.pnl))}
                    >
                      {h.pnl > 0 ? "+" : ""}
                      {fmt(h.pnl)}
                    </td>
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div>
            ...
            <input
              value={cCode}
              onChange={(e) =>
                setCCode(e.target.value.replace(/\D/g, "").slice(0, 6))
              }
              placeholder="6 位代码"
          ...
          <div>
            ...
            <input
              value={cShares}
              onChange={(e) =>
                setCShares(e.target.value.replace(/[^\d.]/g, ""))
              }
              placeholder="如 100"
              className="w-24 rounded-lg border border-border bg-black/20 px-3 py-2 text-sm outline-none focus:border-primary/50"
            />
              ...
              <tbody>
                {closed.map((c, i) => (
                  <tr key={i} className="border-b border-border/30">
                    ...
                    <td
                      className={cn("px-2 py-2.5 font-mono", pnlColor(c.pnl))}
                    >
                      {c.pnl > 0 ? "+" : ""}
                      {fmt(c.pnl)}
                    </td>
