...
export function MorningView({
  ...
  const loadRanks = useCallback((date?: string, clear = false) => {
    ...
    Promise.allSettled([
      api.marginStockRank(10, date),
      api.marginSectorRank(10, date),
    ])
      .then(([s, b]) => {
              ...
              {watchCodes.map((c) => {
                if (isAShare(c)) {
                  ...
                  return (
                    <div
                      key={c}
                      className="group relative rounded-lg bg-muted/25 p-3"
                    >
                      <button
      ...
      {!showOnlyWatchlist && (
        <>
          ...
          <section>
            ...
            {/* 行业融资排名 */}
            <h4 className="mb-2 text-xs font-semibold text-muted-foreground/70">
              行业板块 Top10
            </h4>
            ...
            {/* 6. 个股融资净买入/卖出前 10 */}
            <h4 className="mb-2 mt-6 text-xs font-semibold text-muted-foreground/70">
              个股 Top10
            </h4>
            <div className="grid gap-4 md:grid-cols-2">
              {[
